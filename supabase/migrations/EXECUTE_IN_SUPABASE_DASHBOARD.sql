...
-- ========================================
-- Fix User Deletion Issues - Complete Script
-- ========================================
...
-- 2. Drop existing foreign key constraints that reference users
DO $$
BEGIN
  ...
  -- Check if other tables exist and drop their constraints
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'counter_offers' AND table_schema = 'public') THEN
    ...
    EXCEPTION WHEN OTHERS THEN
      RAISE NOTICE 'counter_offers constraints do not exist or already dropped';
    END;
  END IF;
  ...
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'transactions' AND table_schema = 'public') THEN
    ...
    EXCEPTION WHEN OTHERS THEN
      RAISE NOTICE 'transactions constraints do not exist or already dropped';
    END;
  END IF;
  ...
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'auction_watchers' AND table_schema = 'public') THEN
    ...
    EXCEPTION WHEN OTHERS THEN
      RAISE NOTICE 'auction_watchers constraints do not exist or already dropped';
    END;
  END IF;
END $$;

-- 3. Recreate constraints with proper CASCADE behavior
DO $$
BEGIN
  ...
  -- Notifications table constraints (already has CASCADE in schema)
  -- This might already exist with CASCADE, so we'll handle the error gracefully
  BEGIN
  ...
  RAISE NOTICE 'Main constraints recreated successfully';
END $$;

-- 4. Handle optional tables if they exist
DO $$
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'counter_offers' AND table_schema = 'public') THEN
    ...
    RAISE NOTICE 'counter_offers constraints added';
  END IF;
  ...
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'transactions' AND table_schema = 'public') THEN
    ...
    RAISE NOTICE 'transactions constraints added';
  END IF;
  ...
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'auction_watchers' AND table_schema = 'public') THEN
    ...
    RAISE NOTICE 'auction_watchers constraints added';
  END IF;
END $$;

-- 5. Create a function to safely delete users
...
BEGIN
  ...
  BEGIN
    ...
    -- 2. Remove user as winner from any auctions (will be handled by SET NULL constraint)
    UPDATE public.auctions 
    SET winner_id = NULL 
...
-- 6. Create a preview function to see what would be deleted
...
SELECT 'Migration completed successfully! You can now delete users safely.' AS status;
