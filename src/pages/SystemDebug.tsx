...
export default function SystemDebug() {
    ...
    // Test 9: Repair User Profile
    const repairUserProfile = async () => {
        if (!user) {
            addResult({
                test: 'Repair User Profile',
                status: 'error',
                message: 'User must be logged in to repair profile',
                details: null
            });
            return;
        }
        ...
        try {
            ...
            if (checkError && checkError.code === 'PGRST116') {
                ...
                addResult({
                    test: 'Repair User Profile',
                    status: 'error',
                    message: `Error checking user profile: ${checkError.message}`,
                    details: checkError
                });
            } else {
                addResult({
                    test: 'Repair User Profile',
                    status: 'success',
                    message: 'User profile already exists and is correct',
            ...
            addResult({
                test: 'Repair User Profile',
                status: 'error',
                message: `User profile repair failed: ${error}`,
                details: error
            });
        }
    };
    ...
    return (
        <div className="container mx-auto p-6">
            ...
            <Alert className="mb-6 border-l-4 border-red-500">
                <AlertDescription>
                    <div className="font-semibold text-red-700 mb-2">🚨 CRITICAL ISSUE DETECTED</div>
                    <p className="text-red-600 mb-2">
                        <strong>Foreign Key Constraint Error:</strong> Users authenticated with Supabase Auth but missing from users table.
                    </p>
                    <p className="text-sm text-gray-700 mb-3">
                        This prevents bidding functionality. The database trigger for auto-creating user profiles may not be working properly.
                    </p>
                    <div className="flex items-center gap-2">
                        <Badge className="bg-red-100 text-red-800">Error Code: 23503</Badge>
                        <span className="text-sm">→ Click "🔧 Repair User" button below to fix this issue</span>
                    </div>
                </AlertDescription>
            </Alert>
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-4">
                            <div className="flex gap-2 flex-wrap">
                                ...
                                <Button onClick={repairUserProfile} variant="secondary">🔧 Repair User</Button>
